%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this program will calculate the HW 1 with aid of Total Lagrangian       %
% Description and Modified Newton-Raphson method                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;
clear all;
format long;
...
while (abs((current_coordinate(2*node2-1,1)-current_coordinate(2*node1-1,1)))>22)
        ...
        for i=1:4
            ...
            for j=1:4
                ...
                for gp=1:4
                    ...
                    [ N0,DNX0,DNY0,J0] = Shape_functions( xi,yi,initial_element_coordinate );
                    ...
                    S(:,gp)=D*strain_vector;
                    S_tensor(1,1)=S(1,gp);
                    S_tensor(2,2)=S(2,gp);
                    S_tensor(1,2)=0.5*S(3,gp);
                    S_tensor(2,1)=S_tensor(1,2);
                    sigma_tensor=1/det(F)*F*S_tensor*F';
                    sigma_vector(1,1)=sigma_tensor(1,1);
                    sigma_vector(2,1)=sigma_tensor(2,2);
                    sigma_vector(3,1)=2*sigma_tensor(1,2);
                    sigma_global(4*el-4+gp,1:3)=sigma_vector';
    ...
    output_deflection(load_step_counter,1)=U(26,1);
